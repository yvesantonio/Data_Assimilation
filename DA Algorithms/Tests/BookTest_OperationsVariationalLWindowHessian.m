...
for L=L_min:L_max
    ...
    for i=2:99
        x_f = M(i, x_a{L}(:, i-1));
        x_a{L}(:, i) = da_var_4DVarStrong(x_f, y(:,i-1:i+L-1), M, J_M, H, J_H, B, R, @(f, x0) PolakRibiere(f, x0, 1e-2, 10));
        v = (x_a{L}(:, i) - x_f);
        ...
        for j=0:L-1
            ...
            He = He + 0.9*He_proj'*J_H(i+j+1, xf)'*(R^-1)*J_H(i+j+1, xf)*He_proj;
        end
        B = J_M(i+1, x_a{L}(:, i))'*He^-1*J_M(i+1, x_a{L}(:, i));
        % eig(B)
        % pause
        i
    end
...
save d4DVarTestL1-50Iter10B_Hessian